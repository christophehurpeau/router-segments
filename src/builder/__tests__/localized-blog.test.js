...
describe('localized blog', () => {
    ...
    (segmentBuilder: SegmentRouterBuilderType) => {
      ...
      segmentBuilder.addLocalizedSegment(
        ...
        (segmentBuilder: SegmentRouterBuilderType) => {
          ...
          segmentBuilder.addSegment('/:term', (segmentBuilder: SegmentRouterBuilderType) => {
  ...
  describe('postList', () => {
    describe('route', () => {
      const rrPostList = router.get('postList');
  ...
  describe('postView', () => {
    ...
    describe('find', () => {
      ...
      test('fr', () => {
        ...
        expect(match.path).toBe(frPath);
        expect(match.route).toBe(router.get('postView'));
        expect(match.namedParams).toEqual(new Map([['id', '001'], ['slug', 'Le-Premier-Article']]));
        expect(match.otherParams).toBe(undefined);
      });
    });
  });
  ...
  describe('postWithTag', () => {
    describe('route', () => {
      const rrPostWithTag = router.get('postWithTag');

      test('en', () => {
        const routePath = rrPostWithTag.localizedPaths.get('en');
        expect(routePath.namedParams).toEqual(['tag', 'date', 'slug']);

        expect(routePath.toPath({ tag: 'test', date: '2017-01-01', slug: 'The-First-Post' })).toBe(
          '/post/test/2017-01-01_The-First-Post',
        );
      });
      ...
      test('fr', () => {
        ...
        expect(
          routePath.toPath({ tag: 'test', date: '2017-01-01', slug: 'Le-Premier-Article' }),
    ...
    describe('find', () => {
      ...
      test('en', () => {
        ...
        expect(match.namedParams).toEqual(
          new Map([['tag', 'tag1'], ['date', '2015-01-01'], ['slug', 'The-First-Post']]),
        );
        expect(match.otherParams).toBe(undefined);
      });
      ...
      test('fr', () => {
        ...
        expect(match.route).toBe(router.get('postWithTag'));
        expect(match.namedParams).toEqual(
          new Map([['tag', 'tag1'], ['date', '2015-01-01'], ['slug', 'Le-Premier-Article']]),
        );
        expect(match.otherParams).toBe(undefined);
      });
    });
  ...
  describe('find search-results-tag', () => {
    ...
    test('en', () => {
      ...
      expect(match.namedParams).toEqual(new Map([['term', 'termsearched'], ['tag', 'tag1']]));
    });
