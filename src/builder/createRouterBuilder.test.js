...
test('should throw when key is used twice', () => {
  ...
  expect(() => builder.add('/path1', ref, 'samekey')).toThrow('"samekey" is already used');
});
...
test('should throw when add localized is called but no locales were defined', () => {
  ...
  expect(() => builder.addLocalized({ en: '/path1' }, ref)).toThrow('Invalid locales');
  expect(() => builder.addLocalizedSegment({ en: '/path1' }, () => {})).toThrow('Invalid locales');

  expect(() =>
    builder.addSegment('/path1', (segmentBuilder: SegmentRouterBuilderType) => {
...
describe('default key is path', () => {
  ...
  test('segment add', () => {
    const builder = createRouterBuilder();
    builder.addSegment('/sgmt', (segmentBuilder: SegmentRouterBuilderType) => {
